...
import { QuickApplyTrigger } from '@/components/QuickApplyModal'
...
export default function JobCard({ job }: JobCardProps) {
  ...
  return (
    // Wrapper is `relative` so action buttons can be absolutely positioned outside the <Link>
    <div className="relative group">
      <Link href={`/jobs/${job.slug}`} className="block">
        ...
        >
          {/* Badges row */}
          {(job.is_featured || job.is_seasonal || isNew || hasQuickApply) && (
          ...
          {/* Quick apply action row (inside card for layout, but QuickApplyTrigger is outside the Link) */}
          {hasQuickApply && (
            <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between">
              <p className="text-xs text-slate-500">Αποστολή βιογραφικού απευθείας στην εταιρεία</p>
              {/* Spacer — the real button is positioned outside the Link below */}
              <div className="w-28" />
            </div>
          )}
        </article>
      ...
      {/* Save button — positioned outside <Link> to avoid button-in-anchor */}
      <div className="absolute top-3 right-3 z-10">
        <SaveButton jobId={job.id} />
      </div>

      {/* Quick apply button — also outside <Link> */}
      {hasQuickApply && (
        <div className="absolute bottom-3 right-4 z-10">
          <QuickApplyTrigger job={job} />
        </div>
      )}
    </div>
  )
